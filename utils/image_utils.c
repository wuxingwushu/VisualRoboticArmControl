...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    if (use_handle) {
        if (src_phy != NULL) {
            uintptr_t ptr = (uintptr_t)src_phy;
            rga_handle_src = importbuffer_physicaladdr((uint64_t)ptr, &in_param);
        } else if (src_fd > 0) {
    ...
    if (use_handle) {
        if (dst_phy != NULL) {
            uintptr_t ptr = (uintptr_t)dst_phy;
            rga_handle_dst = importbuffer_physicaladdr((uint64_t)ptr, &dst_param);
        } else if (dst_fd > 0) {
